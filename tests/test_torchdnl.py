...
def test_basic_movement():
    ...
    dnl = TorchDNL(
        edge_static, paths, device="cpu", stuck_threshold=10, edge_endpoints=edge_endpoints
    )
...
def test_spillback():
    ...
    # 2 Agents
    paths = torch.tensor([[0, 1, -1], [0, 1, -1]], dtype=torch.long)
...
def test_capacity_flow():
    ...
    # 2 Edges, flow cap = 2
    edge_static = torch.tensor([[100, 10, 100, 2, 2], [100, 10, 100, 2, 2]], dtype=torch.float32)
...
def test_force_spillback_entry():
    ...
    # stuck_threshold = 2
    dnl = TorchDNL(
        edge_static, paths, device="cpu", stuck_threshold=2, edge_endpoints=edge_endpoints
    )
...
def test_disconnected_abort():
    ...
    edge_static = torch.tensor(
        [[100, 10, 10, 3600, 1], [100, 10, 10, 3600, 1], [100, 10, 10, 3600, 1]],
        dtype=torch.float32,
    )
...
def test_event_tracking():
    ...
    # Same as Test 1
    edge_static = torch.tensor([[10, 10, 10, 3600, 1], [10, 10, 10, 3600, 1]], dtype=torch.float32)
    paths = torch.tensor([[0, 1, -1]], dtype=torch.long)
    edge_endpoints = torch.tensor([[0, 1], [1, 2]], dtype=torch.int32)
