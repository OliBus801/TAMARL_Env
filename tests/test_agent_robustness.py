...
def test_replicator_dynamics_nan_prior_fallback():
    ...
    agent = ReplicatorDynamicsAgent(
        num_agents=2, k_paths=3, prior_mean=prior_mean, beta=0.1, epsilon_start=0.1, device="cpu"
    )
...
def test_replicator_dynamics_all_zero_masks():
    agent = ReplicatorDynamicsAgent(num_agents=2, k_paths=3, epsilon_start=0.1, device="cpu")
...
def test_exp3_agent_all_zero_masks():
    agent = Exp3Agent(num_agents=2, k_paths=3, gamma=0.1, device="cpu")
...
def test_msa_agent_all_zero_masks():
    ...
    agent = MSAAgent(env=env, k_paths=3, device="cpu")
