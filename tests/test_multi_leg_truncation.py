...
def main():
    ...
    print(f"Running simulation with fixed routes (Top-1 path)...")
    ...
    ff_tt_steps = dnl.ff_travel_time_steps # [E]
    ...
    for agent_idx in not_arrived_indices.tolist():
        ...
        for leg_idx in range(num_legs):
            ...
            if agent_idx == 0:
                print(f"Agent {agent_idx} Leg {leg_idx}: status={leg_status_str}, realized_tt={realized_tt:.1f}, FFTT={path_fftt:.1f}")
            ...
            if realized_tt > 0 and realized_tt < path_fftt:
                violation_count += 1
                if agent_idx != 0:
                    print(f"Agent {agent_idx} Leg {leg_idx}: status={leg_status_str}, realized_tt={realized_tt:.1f}, FFTT={path_fftt:.1f} (VIOLATION)")
