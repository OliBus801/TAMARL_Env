...
def main():
    ...
    parser.add_argument(
        "--files", type=str, nargs="+", required=True, help="List of .pkl files to compare"
    )
    parser.add_argument(
        "--labels",
        type=str,
        nargs="+",
        help="Labels for the methods (optional, defaults to filenames)",
    )
    parser.add_argument(
        "--population", type=str, default=None, help="Population filter (e.g. '100' or '1pct')"
    )
    parser.add_argument(
        "--population_file", type=str, default=None, help="Path to a specific population XML file"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="results",
        help="Directory to save the generated plots and maps",
    )
    ...
    labels = (
        args.labels
        if args.labels
        else [os.path.basename(f).replace(".pkl", "") for f in args.files]
    )
    ...
    # Map edge index to geometry nodes
    ...
    for file_idx, file_path in enumerate(args.files):
        ...
        results[label] = {"metrics": metrics, "load": load_tensor.numpy(), "paths_dict": paths_dict}
    ...
    plt.grid(True, alpha=0.3)
    plt.savefig(
        os.path.join(args.save_dir, "dissimilarity_vs_length.png"), dpi=300, bbox_inches="tight"
    )
    if args.show:
        plt.show()
    plt.close()
    ...
    for label, data in results.items():
        ...
        if active_geoms:
            gdf = gpd.GeoDataFrame(
                {"edge_id": active_indices, "load": active_loads}, geometry=active_geoms
            )
            ...
            gdf.plot(
                column="load",
                cmap="hot_r",
                linewidth=gdf["load"] / gdf["load"].max() * 5,
                ax=ax,
                legend=True,
            )
            ...
            ax.set_axis_off()
            plt.savefig(
                os.path.join(args.save_dir, f"heatmap_{label}.png"), dpi=300, bbox_inches="tight"
            )
            if args.show:
                plt.show()
            plt.close()
        ...
        for label, data in results.items():
            ...
            for p_idx, path in enumerate(paths):
                path_geoms = [
                    edge_geometries[eid]
                    for eid in path
                    if eid < len(edge_geometries) and edge_geometries[eid] is not None
                ]
                if path_geoms:
            ...
            custom_lines.append(
                Line2D([0], [0], marker="o", color="red", linestyle="", markersize=10)
            )
            ...
            plt.savefig(
                os.path.join(
                    args.save_dir, f"k_paths_OD_{sample_od[0]}_{sample_od[1]}_{label}.png"
                ),
                dpi=300,
                bbox_inches="tight",
            )
