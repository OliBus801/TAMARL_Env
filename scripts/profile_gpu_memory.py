...
from tests.benchmark_torchdnl import parse_network, parse_population
...
def run_profiling(
    ...
    if expandable_segments:
        ...
        print(
            "Note: PYTORCH_ALLOC_CONF=expandable_segments:True should be set before running the script."
        )

    # 1. Locate files
    files = [f for f in os.listdir(root_folder) if f.endswith(".xml")]
    network_file = next(
        (os.path.join(root_folder, f) for f in files if "network" in f.lower()), None
    )
    ...
    print(f"\n--- Instantiating TorchDNL on {device} ---")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "root_folder", help="Root folder containing network and population xml files"
    )
    parser.add_argument("--population", help="Population filter", default=None)
    parser.add_argument(
        "--use_float16", action="store_true", help="Cast heavy arrays to float16 to test impact"
    )
    parser.add_argument(
        "--expandable_segments",
        action="store_true",
        help="Set PYTORCH_ALLOC_CONF=expandable_segments:True",
    )
    parser.add_argument(
        "--steps", type=int, default=1000, help="Number of steps to run in profiler"
    )
