...
def replot_vc_ratio(peak_csv, max_csv, output_path):
    if not os.path.exists(peak_csv) or not os.path.exists(max_csv):
        print(f"  ⚠ Missing Plot 2 CSV files.")
        return
    ...
    ax1.hist(vc_peak, bins=bins_vc, color=_ACCENT_CYAN, alpha=0.85, edgecolor='none', label='Peak interval')
    ax1.axvline(x=1.0, color=_ACCENT_RED, linestyle='--', linewidth=1.5, alpha=0.8, label='V/C = 1.0 (capacity)')
    ...
    ax2.hist(max_vc, bins=bins_max, color=_ACCENT_ORANGE, alpha=0.85, edgecolor='none', label='Max V/C per edge')
    ax2.axvline(x=1.0, color=_ACCENT_RED, linestyle='--', linewidth=1.5, alpha=0.8, label='V/C = 1.0 (capacity)')
...
def replot_nash_convergence(history_csv, output_path):
    ...
    if len(eps_with_regret) > 0:
        ax.plot(eps_with_regret, mean_min, color=_ACCENT_CYAN, linewidth=2.0, label='Mean Regret', alpha=0.9)
        ax.fill_between(eps_with_regret, np.maximum(0, mean_min - std_min), mean_min + std_min, color=_ACCENT_CYAN, alpha=0.2, edgecolor='none')
        ax.plot(eps_with_regret, max_min, color=_ACCENT_RED, linewidth=1.5, label='Max Regret', alpha=0.7)
        ...
        if len(eps_compliance) > 0:
            ...
            ax_twin.plot(eps_with_regret, eps_compliance * 100, color=_ACCENT_GREEN, linewidth=1.5, linestyle='--', label='ε-Compliance', alpha=0.8)
            ...
            ax.legend(lines1 + lines2, labels1 + labels2, loc='upper right', fontsize=9, framealpha=0.8, facecolor=_DARK_CARD, edgecolor=_GRID_COLOR, labelcolor=_TEXT_COLOR)
        else:
            ax.legend(fontsize=9, framealpha=0.8, facecolor=_DARK_CARD, edgecolor=_GRID_COLOR, labelcolor=_TEXT_COLOR)
    else:
        ax.text(0.5, 0.5, 'No regret data available', transform=ax.transAxes, ha='center', va='center', color=_TEXT_COLOR, fontsize=11)
...
def main():
    parser = argparse.ArgumentParser(description="Regenerate Sanity Check Plots from existing CSV files.")
        ...
        if os.path.exists(csv_1):
            ...
            if args.recompute_ff:
                ...
                if scenario_path and os.path.exists(scenario_path):
                    ...
                    print(f"  ⚠ Could not infer scenario path to recompute FFTT. Make sure the directory is within scenarios/...")
