...
def load_network(network_file):
    ...
    try:
        ...
        print("Error: Could not import tamarl.envs.scenario_loader. Please run from the TAMARL_Env root.")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="Investigate Free-Flow violations in sanity check CSV using Top-K paths.")
    ...
    parser.add_argument("--population", type=str, default=None, help="Path to the population XML file")
    ...
    if not network_file or not pop_file or not paths_file:
        ...
        if not pop_file:
            pops = [f for f in files if ('population' in f.lower() or 'plans' in f.lower()) and f.endswith('.xml')]
            routed = [p for p in pops if 'routed' in p.lower()]
            pop_file = os.path.join(scenario_path, routed[0] if routed else pops[0]) if pops else None
        if not paths_file:
    ...
    if not pd.isna(chosen_path_idx) and chosen_path_idx >= 0:
        ...
        if chosen_path_idx < len(paths):
            ...
            print(f"⚠ Chosen path index {chosen_path_idx} is out of bounds for top-{len(paths)} paths!")
            path_list = paths
            offset = 0
    else:
