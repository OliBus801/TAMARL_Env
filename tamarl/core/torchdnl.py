...
class TorchDNL:
    def __init__(
        ...
        Args:
            ...
            track_events: If True, records simulation events in a MATSim-compatible format.
        ...
        if act_end_times is not None:
            ...
            self.act_end_times = torch.full(
                (self.num_agents, 0), -1, device=device, dtype=torch.int32
            )

        if act_durations is not None:
            self.act_durations = act_durations.to(device).int()
        else:
            self.act_durations = torch.full(
                (self.num_agents, 0), -1, device=device, dtype=torch.int32
            )
        ...
        self.ff_travel_time_steps = (
            torch.floor(self.edge_static[:, 4] / self.dt).long().contiguous()
        )

        # Edge Dynamic State [E]
        self.edge_occupancy = torch.zeros(self.num_edges, device=self.device, dtype=torch.int32)

        # Flow Capacity Accumulator (lazy on-demand model)
        self.edge_capacity_accumulator = self.flow_capacity_per_step.clone().to(
        ...
        if self.collect_link_tt:
            ...
            self.interval_tt_sum = torch.zeros(
                (num_intervals, self.num_edges),
                device="cpu",
                dtype=torch.float32,
                pin_memory=pin_memory,
            )
            self.interval_tt_count = torch.zeros(
                (num_intervals, self.num_edges),
                device="cpu",
                dtype=torch.float32,
                pin_memory=pin_memory,
            ...
            self.current_interval_tt_sum = torch.zeros(
                self.num_edges, device=self.device, dtype=torch.float32
            )
            self.current_interval_tt_count = torch.zeros(
                self.num_edges, device=self.device, dtype=torch.float32
            )
        ...
        if departure_times is not None:
            ...
            self.departure_times = torch.zeros(
                self.num_agents, device=self.device, dtype=torch.int32
            )
        ...
        self.leg_metrics = torch.zeros(
            (self.num_agents, self.max_legs_count, 2), device=self.device, dtype=torch.float32
        )
        # Leg departure times [A, MaxLegs] -> Actual step when agent departed for that leg
        self.leg_departure_times = torch.full(
            (self.num_agents, self.max_legs_count), -1, device=self.device, dtype=torch.int32
        )
        ...
        self._edge_priority_scratch = torch.zeros(
            self.num_edges, device=self.device, dtype=torch.float32
        )
    ...
    def _init_event_buffer(self):
        ...
        if self.track_events:
            ...
            self._event_buffer = torch.empty(
                (self._event_chunk_size, 4), device=self.device, dtype=torch.int32
            )
            self._event_count = 0
    ...
    # =========================================================================
    # Flow Accumulation (updateFastFlowAccumulation)
    # =========================================================================
    def _update_all_flow_accumulation(self):
        ...
        self.edge_capacity_accumulator = torch.minimum(
            self.edge_capacity_accumulator + self.flow_capacity_per_step, remaining
        )
    ...
    def _process_nodes_A(self):
        ...
        if disconnected.any():
            ...
            self.leg_metrics[stuck_agents, c_legs_stuck, 1] = (
                self.current_step - dep_times
            ).float()
            # OPT: Update buffer_counts for agents leaving buffer
            self.buffer_counts.scatter_add_(
                0, m_curr[disconnected], -torch.ones(num_stuck, device=self.device)
            )
        ...
        # queue-model: emptyBufferAfterBufferRandomDistribution
        ...
        if winners.numel() > 0:
            ...
            self.buffer_counts.scatter_add_(
                0, w_curr, -torch.ones(winners.size(0), device=self.device)
            )

            # Update occupancy: add to downstream link
            self.edge_occupancy += torch.bincount(w_next, minlength=self.num_edges).to(
                self.edge_occupancy.dtype
            )
    ...
    def _process_links_B(self):
        ...
        flow_limits = torch.ceil(
            torch.clamp(self.edge_capacity_accumulator[edges_sorted] - 1e-6, min=0.0)
        )
        ...
        self.edge_occupancy -= torch.bincount(all_proc_edges, minlength=self.num_edges).to(
            self.edge_occupancy.dtype
        )
    ...
    def _schedule_demand_C(self):
        ...
        if exiters.numel() > 0:
            ...
            if done_agents.numel() > 0:
                ...
                self.leg_metrics[done_agents, done_c_legs, 1] = (
                    self.current_step - dep_times
                ).float()
            ...
            if cont_agents.numel() > 0:
                ...
                self.leg_metrics[cont_agents, c_legs_cont, 1] = (
                    self.current_step - dep_times_cont
                ).float()
        ...
        # Note : The engine's behaviour is that agents enter capacity buffer directly on first link
        self.status[winners] = 2
    ...
    def step(self) -> None:
        ...
        if self.collect_link_tt:
            ...
            if next_interval_idx > self.current_interval_idx:
                ...
                self.interval_tt_sum[self.current_interval_idx].copy_(
                    self.current_interval_tt_sum, non_blocking=True
                )
                self.interval_tt_count[self.current_interval_idx].copy_(
                    self.current_interval_tt_count, non_blocking=True
                )
    ...
    def print_stats(self, sort: str = "cumtime", limit: int = 20) -> None:
        ...
        if self.stuck_count > 0:
            print(
                f"⚠️  [WARNING] | {self.stuck_count} agents stuck/removed | Action: Verify paths/network"
            )
