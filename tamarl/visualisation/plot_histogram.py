...
# Event type constants (mirrored from torchdnl.py)
EVT_DEPARTURE = 1
EVT_ARRIVAL = 6
EVT_STUCKANDABORT = 8
...
def _compute_leg_histogram_data(events, max_steps, dt=1.0, bucket_size_sec=300):
    ...
    stuck_counts, _ = (
        np.histogram(stuck_times, bins=bins)
        if len(stuck_times) > 0
        else (np.zeros(len(bins) - 1, dtype=int), None)
    )
...
def plot_leg_histogram(
    events, max_steps, dt=1.0, bucket_size_sec=300, output_file="leg_histogram.png"
):
    """
    ...
    plt.step(
        x_axis, dep_counts, where="post", label="Departures", color="red", linewidth=2, alpha=0.7
    )
    plt.step(
        x_axis, arr_counts, where="post", label="Arrivals", color="blue", linewidth=2, alpha=0.7
    )
    plt.step(
        x_axis,
        en_route_counts,
...
def export_leg_histogram_csv(
    events, max_steps, dt=1.0, bucket_size_sec=300, output_file="leg_histogram.csv"
):
    """
...
def plot_leg_histogram_from_arrays(
    start_steps, arrival_steps, max_steps, bucket_size_sec=300, output_file="leg_histogram.png"
):
    """
    ...
    plot_leg_histogram(
        events, max_steps, dt=1.0, bucket_size_sec=bucket_size_sec, output_file=output_file
    )
