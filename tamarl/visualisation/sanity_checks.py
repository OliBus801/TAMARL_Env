...
def plot_tt_vs_fftt(
    ...
    output_path: str,
    original_idx: np.ndarray | None = None,
    planned_dep: np.ndarray | None = None,
    actual_dep: np.ndarray | None = None,
    arrival: np.ndarray | None = None,
    chosen_path_idx: np.ndarray | None = None,
):
    ...
    ff_np = (
        fftt_chosen.astype(np.float64)
        if isinstance(fftt_chosen, np.ndarray)
        else fftt_chosen.numpy().astype(np.float64)
    )
    ...
    try:
        with open(csv_path, "w", newline="") as f:
            ...
            for i in range(len(tt_np)):
                ...
                writer.writerow(
                    [idx, ff_np[i], tt_np[i], ff_min[i], tt_min[i], pd_val, ad_val, ar_val, cp_val]
                )
...
def plot_vc_ratio(
    ...
    ax2.text(
        0.98,
        0.98,
        f"edges > 1.0 : {pct_over_max:.1f}%\nMedian max V/C: {np.median(max_vc):.2f}",
...
def plot_nash_convergence(
    regret_history: list[tuple[float | None, float | None, float | None, float | None]],
    output_path: str,
):
...
def plot_route_allocation_heatmap(
    ...
    im = ax.imshow(
        ...
        cmap=cmap,
        origin="lower",
        extent=[0, num_episodes, -0.5, num_routes - 0.5],
        vmin=0,
        vmax=1,
    )
    ...
    ax.set_title(
        f"Route Allocation Heatmap (OD {major_od_idx}, {major_od_count} agents)",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_entropy_scatter(
    ...
    mean_tt: float,
    output_path: str,
    unique_od: np.ndarray | None = None,
    ...
    try:
        with open(raw_csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["leg_idx", "od_idx", "origin_node", "destination_node", "chosen_path_idx"]
            )
...
def plot_lorenz_concentration(
    ...
    ax.plot(
        x_routes, cumulative_p, marker="o", color=_ACCENT_GREEN, linewidth=2, label="Current Run"
    )

    # Random Uniform Line
    ax.plot(
        x_routes,
        np.cumsum(np.ones(K) / K) * 100,
        linestyle="--",
        color=_TEXT_COLOR,
        alpha=0.5,
    ...
    ax.set_ylim(0, 105)
    ax.set_title(
        f"Route Concentration Profile (OD {major_od_idx}, {major_od_count} agents)",
        fontsize=12,
        fontweight="bold",
    )
    ax.legend(
...
def plot_spatiotemporal_violins(
    ...
    if valid_data:
        parts = ax.violinplot(
            valid_data, positions=valid_hours, showmeans=False, showextrema=True, showmedians=True
        )
    ...
    ax.set_title(
        "Spatio-Temporal Distribution of Load (Hourly Violins)", fontsize=12, fontweight="bold"
    )
    if valid_hours:
...
def plot_critical_corridors_heatmap(
    ...
    top_100_edges = top_100_edges[
        edge_counts[top_100_edges] > 0
    ]  # Filter out edges with 0 frequency
    ...
    try:
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            time_labels = [
                f"t_{int((i * link_tt_interval) // 60)}" for i in range(heatmap_data.shape[1])
            ]
    ...
    ax.set_title(
        "Spatiotemporal Corridors Heatmap (Top Critical Links)", fontsize=12, fontweight="bold"
    )
...
def generate_sanity_checks(
    ...
    regret_history: list[tuple[float | None, float | None, float | None, float | None]],
    output_dir: str,
    route_allocation_history: list[np.ndarray] | None = None,
    scenario_id: str = "",
    agent_type: str = "",
):
    ...
    if (
        "chosen_path_idx" in best_sanity_data
        and "od_indices" in best_sanity_data
        and best_sanity_data["od_indices"] is not None
    ):
        plot_entropy_scatter(
            chosen_paths=best_sanity_data["chosen_path_idx"].numpy(),
            od_indices=best_sanity_data["od_indices"].numpy(),
            original_leg_idx=best_sanity_data.get(
                "original_leg_idx", np.arange(len(best_sanity_data["chosen_path_idx"]))
            ),
