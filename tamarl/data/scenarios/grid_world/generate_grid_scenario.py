...
def write_population_xml(
    ...
    block_size: float = 1000.0,
    seed: int = 42,
    o_mean: tuple[float, float] = None,
    o_std: float = None,
    d_mean: tuple[float, float] = None,
    d_std: float = None,
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Standard deviation as fraction of size (default 0.15 * size)",
    )
    ...
    write_population_xml(
        ...
        block_size=args.block_size,
        seed=args.seed,
        o_mean=(s * 0.25, s * 0.25),
        o_std=std,
        d_mean=(s * 0.75, s * 0.75),
        d_std=std,
    )
