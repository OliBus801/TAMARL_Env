...
def write_population_xml(
    filename: str, num_agents: int, size: int, block_size: float, duration_s: int
):
    ...
    with open(filename, "w") as f:
        ...
        for i in range(num_agents):
            ...
            f.write(
                f'\t\t\t<act type="origin" link="{start_link}" x="{cx_o * block_size:.1f}" y="{cy_o * block_size:.1f}" end_time="{start_time_str}" />\n'
            )
            ...
            f.write("\t\t\t</leg>\n")
            f.write(
                f'\t\t\t<act type="destination" link="{end_link}" x="{cx_d * block_size:.1f}" y="{cy_d * block_size:.1f}" />\n'
            )
            f.write("\t\t</plan>\n")
            f.write("\t</person>\n")
            valid_agents += 1
