...
def load_network(network_file):
    ...
    for link in root.findall(".//link"):
        ...
        links[lid] = {"from": from_node, "to": to_node, "length": length}
    ...
    for person in root.findall("person"):
        ...
        for act in acts:
            ...
            if candidates:
                ...
                print(
                    f"Warning: No incoming links for node {nearest_node_id} (Agent {person.get('id')})"
                )
                assigned_link = (
                    list(G.out_edges(nearest_node_id, data="id"))[0][2]
                    if G.out_degree(nearest_node_id) > 0
                    else None
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--network", default="Siouxfalls_network_PT.xml", help="Path to network XML"
    )
    parser.add_argument(
        "--population", default="Siouxfalls_population.xml", help="Path to input population XML"
    )
    parser.add_argument(
        "--output", default="Siouxfalls_route_population.xml", help="Path to output population XML"
    )
