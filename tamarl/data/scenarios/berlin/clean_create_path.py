...
def process_origin_chunk(tasks):
    ...
    for start_link, targets, start_u, target_nodes in tasks:
        ...
        if limit < 300:
            limit = 300  # 5 mins min

        dist_matrix, predecessors = sp.csgraph.dijkstra(
            global_graph, indices=start_u, return_predecessors=True, limit=limit
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--populations", nargs="+", default=["1pct"], help="Population suffixes (e.g. 1pct 10pct)"
    )
    parser.add_argument(
        "--base_dir",
        default=None,
        help="Base directory containing the scenario files (network, population, facilities)",
    )
    args = parser.parse_args()

    base_dir = args.base_dir or os.path.join(
        os.path.dirname(__file__), "..", "..", "scenarios", "los_angeles"
    )
